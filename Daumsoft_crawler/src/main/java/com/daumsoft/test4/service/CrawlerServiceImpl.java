...
public class CrawlerServiceImpl implements CrawlerService {
	...
	public int count(int type, int category) throws Exception {
		int count = crawlerDao.count(type, category);
	...
	public void getInsightData(String categoryURL) throws Exception {
		...
		try {
			...
			for (int i = 1; i < 100; i++) {
				...
				Document list_doc = Jsoup.connect(URL + "?page=" + page(i)).get(); // URL로 부터 Document를 가져옴
	...
	public void addInsightData(String categoryURL) throws Exception {
		...
		try {
			...
			for (int i = 1; i <= 100; i++) {
				...
				//System.out.println("주제어 : " + category);
				top_idx = crawlerDao.top_idx(type, categoryNum); // 최근에 들어온 글의 idx를 조회
				Elements list_elements = list_doc.select(".section-list li");
				for (Element list_data : list_elements) {
					// 전체 목록 페이지에서 작성자 · 날짜 출력
					info = list_data.select(".section-list-article-byline").text();
	...
	public void getHuffingtonpostData(String categoryURL) throws Exception {
		...
		String idx = null; // 고유 번호
		int type = 2; // 1은 인사이트를 의미함, 2는 허핑턴포스트
		String category = null;
		int categoryNum = 0;
		String title = null;
		String content = null;
		...
		try {
			...
			for (int i = 1; i < 100; i++) {
				...
				for (Element list_data : list_elements) {
					// 전체 목록 페이지에서 작성자 · 날짜 출력
					// System.out.println(list_data.select(".section-list-article-byline").text());
					detailURL = "https://www.huffingtonpost.kr" + list_data.select("a").attr("href");
					// 고유 idx를 가져오기 위함
					...
					title = detail_data.select(".headline__title").text();
					content = detail_data.select("div[class='content-list-component text']").text();
					System.out.println("weekAgo 날짜는 " + weekAgo);
					System.out.println("고유번호 : " + idx);
					System.out.println("사이트 종류 : " + type);
					System.out.println("카테고리 : " + category);
	...
	public void addHuffingtonpostData(String categoryURL) throws Exception {
		...
		try {
			...
			for (int i = 1; i < 100; i++) {
				...
				System.out.println("주제어 : " + category);
				top_idx = crawlerDao.top_idx(type, categoryNum);
				Elements list_elements = list_doc.select("div[class='col col--body-center bnp__card-list yr-col-body-center'] .card__headline");
				for (Element list_data : list_elements) {
					// 전체 목록 페이지에서 작성자 · 날짜 출력
					// System.out.println(list_data.select(".section-list-article-byline").text());
